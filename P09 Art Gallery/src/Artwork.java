...
public class Artwork implements Comparable<Artwork> {
     ...
     *         lower lexical order name than otherArtwork name; String comparisons are
     *         case-sensitive, otherwise returns an integer greater than 0.
     * @author Steven Ren
     */
    @Override
    public int compareTo(Artwork otherArtwork) {
        if (name.equals(otherArtwork.getName()) && cost == otherArtwork.getCost() &&
                year == otherArtwork.getYear()) {
            return 0;
        }
        else if (year < otherArtwork.getYear()) {
            return -1;
        }
        else if (year == otherArtwork.getYear() && cost < otherArtwork.getCost()) {
            return -1;
        }
        else if (year == otherArtwork.getYear() && cost == otherArtwork.getCost() &&
                name.compareTo(otherArtwork.getName()) == -1) {
            return -1;
        }
        else {
            return 1;
    ...
    public boolean equals(Object obj) {
        if (obj instanceof Artwork) {
            if (name.equals(((Artwork) obj).getName()) && year == ((Artwork) obj).getYear()) {
                return true;
            }
        }
