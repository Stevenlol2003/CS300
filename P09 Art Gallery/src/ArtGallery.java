...
public class ArtGallery {
    ...
    public boolean lookup(String name, int year, double cost) {
        ...
        Artwork searchArt = new Artwork(name, year, cost);
        return lookupHelper(searchArt, root);
    }
    ...
    protected static boolean lookupHelper(Artwork target, BSTNode<Artwork> current) {
        // Base case if empty subtree
        if (current == null) {
            return false;
        }
        int comparisonResult = target.compareTo(current.getData());
        // Found match
        if (comparisonResult == 0) {
            return true;
        }
        // Recursive cases
        // If item is less than recursive left
        if (comparisonResult < 0) {
            return lookupHelper(target, current.getLeft());
        }
        // If item is greater than recursive right
        else {
    ...
    public boolean addArtwork(Artwork newArtwork) {
        // if ArtworkGallery was empty create new node and set to root
        if (isEmpty()) {
    ...
    protected static boolean addArtworkHelper(Artwork newArtwork, BSTNode<Artwork> current) {
        ...
        if (compare < 0) {
            // Greater: is the right child null?
    ...
    public Artwork getBestArtwork() {
        Artwork bestArtwork = root.getData();
        BSTNode current = root;

        // keeps on going right until its null then return that right most Artwork
        while (current.getRight() != null) {
            bestArtwork = (Artwork) current.getRight().getData();
    ...
    protected static String toStringHelper(BSTNode<Artwork> current) {
        ...
        // recurse to the left first and sees if any node exists
        toReturn += toStringHelper(current.getLeft());

        // add current node's Artwork to the string
        toReturn += current.getData().toString() + "\n";

        // recurse to the right then and sees if any node exists
    ...
    protected static int heightHelper(BSTNode<Artwork> current) {
        ...
        // recurse to find height of left and right
        int leftHeight = heightHelper(current.getLeft());
        int rightHeight = heightHelper(current.getRight());

        // increments height and returns the max height of left or right
    ...
    protected static ArrayList<Artwork> lookupAllHelper(int year, double cost,
        ...
        Artwork example = new Artwork("Example", year, cost);

        // Compare the target to the data at current and proceed accordingly
        int compare = current.getData().compareTo(example);
        if (compare < 0) {
            artworks.add(current.getData());
        }

        // recurse to the left if left is not null
        if (current.getLeft() != null) {
            artworks.addAll(lookupAllHelper(year, cost, current.getLeft()));
        } else {
            return artworks;
        }
    ...
    protected static BSTNode<Artwork> buyArtworkHelper(Artwork target, BSTNode<Artwork> current)
        ...
        if (current == null) {
            throw new NoSuchElementException("No Artwork found in the BST rooted at current");
        }
        // Compare the target to the data at current and proceed accordingly
        ...
        // if match with target found, three cases should be considered. Feel free to organize
        // the order of these cases at your choice.
        else {
            // current may be a leaf (has no children), set current to null.
            if (current.getLeft() == null && current.getRight() == null) {
                return null;
            }
            // current may have only one child, set current to that child (whether left or right
            // child)
            if (current.getLeft() == null) {
            ...
            // in the tree, and having the same left and right children as current. Notice carefully
            // that you cannot set the data of a BSTNode.
            ...
            // child.
                ...
                Artwork newArtwork = getSuccessor(current);
                BSTNode newNode = new BSTNode(newArtwork, current.getLeft(), current.getRight());
                current = newNode;
    ...
    protected static Artwork getSuccessor(BSTNode<Artwork> node) {
        // keeps on searching left until it's null and then returns the left most key
